...
User.prototype.validate = function () {
  return new Promise(async (resolve, reject) => {
    ...
    if (!validator.isEmail(this.data.email))
      this.errors.push('Voce deve ter um email válido');

    if (this.data.password == '') this.errors.push('Voce deve ter um password');

    if (this.data.password.length > 0 && this.data.password.length < 12)
      this.errors.push('Password com no mminimo 12 caracteres');

    if (this.data.password.length > 50)
      this.errors.push('Password com no maximo 50 caracteres');

    if (this.data.username.length > 0 && this.data.username.length < 3)
      this.errors.push('Username com no minimo 3 caracteres');

    if (this.data.username.length > 30)
      this.errors.push('Username com no maximo 30 caracteres');
    ...
    if (validator.isEmail(this.data.email)) {
      ...
      if (emailExists) {
        this.errors.push('Email já utilizado. :-( ');
      }
    }
    resolve();
  });
